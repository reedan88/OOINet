...
class PHSEN(Instrument):
    """Attributes and methods for reprocessing PHSEN data.

    This code is based on code originally written by C. Wingard
    (chris.wingard@oregonstate.edu)
    """

    def __init__(self):
        """Reformat PHSEN datasets downloaded from OOINet."""
        self.attrs = {
            'unique_id': {
                'long_name': 'Instrument Unique ID',
                'comment': ('One byte checksum summary of the instrument ' +
                            'serial number, name, calibration date and ' +
                            'firmware version serving as a proxy for a ' +
                            'unique ID. While identified as the instrument ' +
                            'unique ID, it is possible for more than one ' +
                            'instrument to have the same checksum summary. ' +
                            'Thus, the uniqueness of this value should be ' +
                            'considered with a certain degree of caution.'),
                # 'units': ''# deliberately left blank, no units for this value
            },
            'measurements': {
                'long_name': 'Measurements Array',
                'comment': 'Dimensional indexing array created for the ' +
                'light measurements.',
                # 'units': ''# deliberately left blank, no units for this value
            },
            'blanks': {
                ...
                'comment': 'Dimensional indexing array created for the ' +
                'reference (blanks) measurements.',
                # 'units': ''# deliberately left blank, no units for this value
            },
            'blank_refrnc_434': {
                'long_name': 'DI Blank Reference Intensity at 434 nm',
                'comment': ('Optical absorbance reference intensity at 434 ' +
                            'nm. Measured with deionized water. Reference ' +
                            'and signal intensities range between 0 and ' +
                            '4096. Values should be greater than ~1500. ' +
                            'Lower intensities will result in higher noise ' +
                            'in the absorbance and pH measurements. ' +
                            'Obtained from the reference_light_measurements ' +
            ...
            'blank_signal_434': {
                ...
                'comment': ('Optical absorbance signal intensity at 434 nm. ' +
                            'Measured with deionized water. Reference and ' +
                            'signal intensities range between 0 and 4096. ' +
                            'Values should be greater than ~1500. Lower ' +
                            'intensities will result in higher noise in the ' +
                            'absorbance and pH measurements. Obtained from ' +
                            'the reference_light_measurements variable in ' +
                            'the Data Portal sourced data file.'),
                'units': 'counts',
                '_FillValue': -9999999,
            },
            'blank_refrnc_578': {
                'long_name': 'DI Blank Reference Intensity at 578 nm',
                'comment': ('Optical absorbance reference intensity at ' +
                            '578 nm. Measured with deionized water. ' +
                            'Reference and signal intensities range between ' +
                            '0 and 4096. Values should be greater than ' +
                            '~1500. Lower intensities will result in higher ' +
                            'noise in the absorbance and pH measurements. ' +
                            'Obtained from the reference_light_measurements ' +
                            'variable in the Data Portal sourced data file.'),
                'units': 'counts',
                '_FillValue': -9999999,
            ...
            'blank_signal_578': {
                ...
                'comment': ('Optical absorbance signal intensity at 578 nm. ' +
                            'Measured with deionized water. Reference and ' +
                            'signal intensities range between 0 and 4096. ' +
                            'Values should be greater than ~1500. Lower ' +
                            'intensities will result in higher noise in the ' +
            ...
            'reference_434': {
                ...
                'comment': ('Optical absorbance reference intensity at ' +
                            '434 nm. Reference and signal intensities range ' +
                            'between 0 and 4096. Values should be greater ' +
                            'than ~1500. Lower intensities will result in ' +
                            'higher noise in the absorbance and pH ' +
                            'measurements. Obtained from the ' +
                            'light_measurements variable in the Data Portal ' +
                            'sourced data file.'),
                'units': 'counts'
            },
            'signal_434': {
                ...
                'comment': ('Optical absorbance signal intensity at 434 nm. ' +
                            ...
                            '~1500. Lower intensities will result in higher ' +
                            'noise in the absorbance and pH measurements. ' +
                            'Obtained from the light_measurements variable ' +
                            'in the Data Portal sourced data file.'),
            ...
            'reference_578': {
                ...
                'comment': ('Optical absorbance reference intensity at ' +
                            '578 nm. Reference and signal intensities range ' +
                            'between 0 and 4096. Values should be greater ' +
                            'than ~1500. Lower intensities will result in ' +
                            'higher noise in the absorbance and pH ' +
                            'measurements. Obtained from the ' +
            ...
            'raw_thermistor_start': {
                ...
                'comment': 'Thermistor resistivity measured in counts at ' +
                           'the start of the measurement cycle.',
                'units': 'counts'
            },
            'raw_thermistor_end': {
            ...
            'raw_battery_voltage': {
                ...
                'comment': ('Raw internal battery voltage measured in ' +
                            'counts. May actually reflect external voltage ' +
                            'if external power is applied'),
                'units': 'counts'
            },
            'thermistor_temperature': {
                'long_name': 'Thermistor Temperature',
                'comment': ('Thermistor temperature refers to the internal ' +
                            'instrument temperature of the pH sensor, as ' +
                            'measured by the thermistor. It is used to ' +
                            'determine salinity and temperature dependent ' +
                            'molar absorptivities in the seawater sample in ' +
                            'order to make an accurate pH estimation. ' +
                            'This variable represents the thermistor ' +
            ...
            'seawater_ph': {
                ...
                'comment': ('pH is a measurement of the concentration of ' +
                            'hydrogen ions in a solution. pH ranges from ' +
                            'acidic to basic on a scale from 0 to 14 with 7 ' +
                            'being neutral.'),
                'standard_name': 'sea_water_ph_reported_on_total_scale',
                'data_product_identifier': 'PHWATER_L2',
                # 'units': ''# deliberately left blank, no units for this value
                'ancillary_variables': ('blank_refrnc_434 blank_signal_434 ' +
                                        'blank_refrnc_578 blank_signal_578 ' +
                                        'reference_434 signal_434 ' +
                                        'reference_578 signal_578 ' +
                                        'thermistor_temperature ' +
                                        'practical_salinity')
            }
        }

    def phsen_instrument(self, ds):
        """Reformat the PHSEN data recorded internally by the instrument.

        Takes PHSEN data recorded by internally by the instrument, and cleans
        ...
        re-organize some of the variables to permit better assessments of the
        data.

        Parameters
        ----------
        ds: (xarray.dataset)
            Initial PHSEN data set recorded internally by the instrument and
        ...
        #   record_time == internal_timestamp == time
        ds = ds.reset_coords()
        ...
        # now we need to reset the light and reference arrays to named
        # variables that will be more meaningful and useful in
        # the final data files
        nrec = len(ds['time'].values)
        light = np.array(np.vstack(ds['light_measurements'].values),
                         dtype='int32')
        ...
        ph = xr.Dataset({
            'blank_refrnc_434': (['time', 'blanks'],
                                 blank_refrnc_434.astype('int32')),
            'blank_signal_434': (['time', 'blanks'],
                                 blank_signal_434.astype('int32')),
            'blank_refrnc_578': (['time', 'blanks'],
                                 blank_refrnc_578.astype('int32')),
            'blank_signal_578': (['time', 'blanks'],
                                 blank_signal_578.astype('int32')),
            'reference_434': (['time', 'measurements'],
                              reference_434.astype('int32')),
            'signal_434': (['time', 'measurements'],
                           signal_434.astype('int32')),
            'reference_578': (['time', 'measurements'],
                              reference_578.astype('int32')),
            'signal_578': (['time', 'measurements'],
                           signal_578.astype('int32'))
        }, coords={'time': ds['time'],
        ...
        ds = ds.drop(['light_measurements', 'reference_light_measurements',
                      'spectrum', 'reference_light_measurements_dim_0'])
        ...
        data_types = ['deployment', 'raw_thermistor_end',
                      'raw_thermistor_start', 'raw_battery_voltage']
        for v in data_types:
            ds[v] = ds[v].astype('int32')

        return ds

    def phsen_datalogger(self, ds):
        """Reformat PHSEN data recorded by the CGSN/EA mooring data loggers.

        Takes PHSEN data recorded by the data loggers used in the CGSN/EA
        moorings and cleans up the data set to make it more user-friendly.
        Primary task is renaming parameters and dropping some that are of
        limited use. Additionally, re-organize some of the variables to
        permit better assessments of the data.

        Parameters
        ----------
        ds: (xarray.datasets)
            Initial PHSEN data set recorded by the data logger system and
            downloaded from OOI via the M2M system

        Returns
        -------
        ds: (xarray.datasets)
            Cleaned up and reorganized data set.
        """
        # drop some of the variables:
        ...
        #   phsen_abcdef_signal_intensity_434, part of the light measurements
        #   phsen_abcdef_signal_intensity_578, part of the light measurements
        ds = ds.reset_coords()
        ds = ds.drop(['passed_checksum', 'record_type', 'record_time',
                      'phsen_abcdef_signal_intensity_434',
                      'phsen_abcdef_signal_intensity_578',
                      'dcl_controller_timestamp'])

        # convert the time values from a datetime64[ns] object to a
        # floating point number with the time in seconds
        ds['internal_timestamp'] = ('time',
                                    self.dt64_epoch(ds.internal_timestamp))
        ds['internal_timestamp'].attrs = dict({
            'long_name': 'Internal SAMI-pH Clock Time',
            'standard_name': 'time',
            'units': 'seconds since 1970-01-01 00:00:00 0:00',
            'calendar': 'gregorian',
            'comment': ('Comparing the instrument internal clock versus the ' +
                        'GPS referenced sampling time will allow for ' +
                        'calculations of the instrument clock offset and ' +
                        'drift. Useful when working with the recovered ' +
                        'instrument data where no external GPS referenced ' +
                        'clock is available.')
        })
        ...
        # now we need to reset the light and reference arrays to named
        # variables that will be more meaningful and useful in the final data
        # files
        nrec = len(ds['time'].values)
        ...
        ds = ds.drop(['phsen_abcdef_signal_intensity_434_dim_0',
                      'phsen_abcdef_signal_intensity_578_dim_0',
                      'reference_light_measurements_dim_0', 'spectrum',
                      'light_measurements', 'reference_light_measurements'])
        ...
        data_types = ['deployment', 'raw_thermistor_end',
                      'raw_thermistor_start', 'unique_id',
                      'raw_battery_voltage']
        for v in data_types:
            ds[v] = ds[v].astype('int32')
    ...
    def phsen_imodem(self, ds):
        """Reformat PHSEN data recorded over the inductive modem line.

        Takes PHSEN data recorded by over the inductive modem line, and cleans
        up the data set to make it more user-friendly. Primary task is renaming
        parameters and dropping some that are of limited use. Additionally,
        re-organize some of the variables to permit better assessments of the
        data.
        ...
        #   phsen_abcdef_signal_intensity_578, part of the light measurements
        ds = ds.reset_coords()
        for variable in ['passed_checksum', 'record_type', 'record_time',
                         'internal_timestamp',
                         'phsen_abcdef_signal_intensity_434',
                         'phsen_abcdef_signal_intensity_578']:
            try:
